...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for it in range(max_iter):
        if it % print_every == 10: print(f"Iteration: {it}")
        sorted_pixels = [[] for _ in range(len(centroids))]
        for h in range(image.shape[0]):
            for w in range(image.shape[1]):
                dists = np.array([
                    np.sqrt(np.sum((image[h, w, :] - centroid)**2))
                    for centroid in centroids
                ])
                sorted_pixels[np.argmin(dists)].append((image[h, w, :]))
        new_centroids = [
            np.mean(cluster, axis=0)
             for cluster in sorted_pixels]
...
def update_image(image, centroids):
    ...
    for h in range(image.shape[0]):
        for w in range(image.shape[1]):
            ...
            image[h, w, :] = centroids[np.argmin(dists)]
